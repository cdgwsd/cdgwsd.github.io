...
function ChucklePostAI(AI_option) {
  ...
  function MAIN(AI_option) {
    ...
    const currentURL = "https://cdgwsd.top" + currentPath;
    // 排除页面
    ...
    async function recommendList() {
      ...
      if(sessionStorage.getItem('recommendList')){
        ...
        try {
          response = await fetch(`https://ai.cdgwsd.top/?url=${encodeURIComponent(window.location.href)}&author=${AI_option.rec_method ? AI_option.rec_method : 'all'}`, options);
    ...
    async function matrixShuttle(){
      ...
      if(sessionStorage.getItem('matrixShuttle')){
        ...
        try {
          response = await fetch('https://ai.cdgwsd.top/websites_used', options);
          completeGenerate = true;
          if (response.status === 429) {
            startAI('请求过于频繁，请稍后再请求AI。');
          }
    ...
    async function summarySpeech(){
      ...
      if(summary_audio && summary_audio){
        ...
        try {
          ai_speech.style.pointerEvents = "none";
          ai_speech.style.opacity = "0.4";
          response = await fetch(`https://ai.cdgwsd.top/audio?${requestParams}`, options);
    ...
    async function ai_init() {
      ...
      ai_btn_item.forEach((item, index) => {
        ...
        if(AI_option.hide_recommend && index === 1){
          item.style.display = 'none';
          return;
        }
    ...
    async function getGptResponse(content, i = true) {
      ...
      if (i) {
        try {
          response = await fetch('https://ai.cdgwsd.top/', {
            signal: signal,
            method: "POST",
            headers: {
              "Content-Type": "application/json",
